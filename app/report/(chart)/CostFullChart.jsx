...
export default function CostFullChart() {
  const router = useRouter();
  const { title, yAxisSuffix = ' ₹', data1, data2, color1 = theme.gridColor, color2 = theme.dgColor } = useLocalSearchParams();
  ...
  const tableData = parsedData1.map((gridItem, index) => {
    ...
    return {
      date: gridItem.label,
      gridCost: gridItem.value,
      dgCost: dgItem.value,
      totalCost: gridItem.value + dgItem.value,
    };
  });
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.legendBox}>
          <View style={styles.legendItem}>
            ...
            <Text style={styles.legendText}>GRID Cost</Text>
          </View>
          <View style={styles.legendItem}>
            <View style={[styles.legendIndicator, { backgroundColor: color2 }]} />
            <Text style={styles.legendText}>DG Cost</Text>
          </View>
        </View>
        ...
        {showTable && (
          <View style={styles.tableContainer}>
            <Text style={styles.tableTitle}>DAILY COST DATA</Text>
            
            {/* Table Header */}
            <View style={styles.tableHeader}>
              <Text style={[styles.tableHeaderText, styles.tableCell, {flex: 1.5}]}>Date</Text>
              <Text style={[styles.tableHeaderText, styles.tableCell, {flex: 1}]}>Grid (₹)</Text>
              <Text style={[styles.tableHeaderText, styles.tableCell, {flex: 1}]}>DG (₹)</Text>
              <Text style={[styles.tableHeaderText, styles.tableCell, {flex: 1}]}>Total (₹)</Text>
            </View>
        ...
        <View style={styles.summaryContainer}>
          <View style={[styles.summaryCard, {borderLeftColor: theme.gridColor}]}>
            ...
            <Text style={[styles.summaryValue, {color: theme.gridColor}]}>
              {totals.grid.toFixed(1)} {yAxisSuffix}
            </Text>
          </View>
          <View style={[styles.summaryCard, {borderLeftColor: theme.dgColor}]}>
            ...
            <Text style={[styles.summaryValue, {color: theme.dgColor}]}>
              {totals.dg.toFixed(1)} {yAxisSuffix}
            </Text>
          </View>
          <View style={[styles.summaryCard, {borderLeftColor: theme.accent}]}>
            ...
            <Text style={[styles.summaryValue, {color: theme.accent}]}>
              {totals.total.toFixed(1)} {yAxisSuffix}
            </Text>
          </View>
        </View>
      </ScrollView>
